...
class RiskAnalyzer(DocumentProcessor):
    ...
    async def process(self, file_path: Union[str, Path], custom_criteria: Optional[List[str]] = None, **kwargs) -> DocumentResult:
        """
        ...
        return DocumentResult.success_result(
            data=result_data,
            message="Анализ рисков успешно завершен"
        )
    ...
    async def _ai_risk_analysis(self, text: str, custom_criteria: Optional[List[str]] = None) -> Dict[str, Any]:
    ...
    def _generate_recommendations(self, pattern_risks: List[Dict[str, Any]], ai_analysis: Dict[str, Any]) -> List[str]:

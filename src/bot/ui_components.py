...
def render_legal_html(
    ...
    *,
    ...
    for em in emoji_markers:
        # Разрываем только если перед эмодзи есть достаточно длинный текст (>30 символов)
        t = re.sub(rf'(?<=\S{{30,}})\s*(?={re.escape(em)})', '\n\n', t)
    ...
    #    - пули/тире (только если предыдущая строка не короткая)
    t = re.sub(r'(?<=\S{20,})\s*(?=[\-–—*]\s+)', '\n\n', t)
    #    - буллит "•" (только если нет соседних bullet)
    t = re.sub(r'(?<![•\-–—*])\s*(?<=\S)\s*•\s*', r'\n• ', t)
    #    - нумерация "1)" или "1." (только начало нового нумерованного списка)
    t = re.sub(r'(?<!\d)(?<=\S{15,})\s*(?=\d{1,2}[\)\.]\s+)', '\n\n', t)

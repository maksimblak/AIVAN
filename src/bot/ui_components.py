...
{Emoji.DOCUMENT} <b>Общая юридическая консультация</b>
• Анализ правовых ситуаций любой сложности
• Разъяснение норм законодательства
• Рекомендации по алгоритму действий
• Оценка рисков и перспектив дела
• Помощь в выборе правовой стратегии
...
def _is_heading(raw_line: str) -> bool:
    ...
    # Короткая строка, заканчивающаяся двоеточием (более строгие критерии)
    if len(clean_s) <= 60 and clean_s.endswith(":") and len(clean_s.split()) <= 8:
        return True
    ...
    # Короткая фраза без точки в конце — вероятный заголовок (более строгие критерии)
    if len(clean_s) <= 50 and not clean_s.endswith((".", "!", "?", ",", ";")) and len(clean_s.split()) <= 6:
        return True
...
def render_legal_html(
    ...
    *,
    ...
    # 1) Разрывы перед «визуальными» маркерами (эмодзи/иконки) — НЕ слова
    emoji_markers = ("📚", "🔸", "▶️", "📋", "⚡️", "🔹", "✅", "📝", "⚖️", "💡", "⚠️")
    for em in emoji_markers:
        t = re.sub(rf'(?<=\S)\s*(?={re.escape(em)})', '\n\n', t)
        # на всякий случай уберём дубликаты подряд
        t = re.sub(rf'{re.escape(em)}\s*{re.escape(em)}+', em, t)

    # 2) Разрывы для списков:
    #    - пули/тире
    t = re.sub(r'(?<=\S)\s*(?=[\-–—*]\s+)', '\n\n', t)
    #    - буллит "•"
    t = re.sub(r'(?<=\S)\s*•\s*', r'\n• ', t)
    #    - нумерация "1)" или "1."
    t = re.sub(r'(?<!\d)(?<=\S)\s*(?=\d{1,2}[\)\.]\s+)', '\n\n', t)
    #    - после ":", если далее начинается список/нумерация — перенос
    t = re.sub(r':\s*(?=(?:•|[\-–—*]|\d{1,2}[\)\.])\s+)', ':\n\n', t)

    # 3) Мягкая сегментация предложений: после . ! ? если дальше заглавная (RU/EN)
    t = re.sub(r'([.!?])\s+(?=[A-ZА-ЯЁ])', r'\1\n\n', t)

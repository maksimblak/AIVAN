...
def _is_heading(raw_line: str) -> bool:
    ...
    # Строки с эмодзи-префиксами уже заголовки
    if re.match(r'^[📋⚖️💡▶️⚠️]', s):
        return True

    # Короткая строка, заканчивающаяся двоеточием
    if len(clean_s) <= 80 and clean_s.endswith(":"):
        return True
    ...
    if len(clean_s) <= 70 and not clean_s.endswith((".", "!", "?")) and len(clean_s.split()) <= 10:
        return True
...
def render_legal_html(
    ...
    *,
    ...
    # 1) БЛОКИ И ЗАГОЛОВКИ - разбиваем на крупные секции
    t = re.sub(r'(?<=\S)\s*(?=\d+\s*[Бб]лок[\.:]?\s*)', '\n\n═══════════════════\n\n', t)

    # 2) ПОДЗАГОЛОВКИ - выделяем важные секции
    t = re.sub(r'(?<=\S)\s*(?=(?:Что именно|Варианты|Нормативная база|Основания|Преимущества|Как оформить|Итого|Рекомендации)[^а-яё])', '\n\n▶️ ', t)

    # 3) СПИСКИ И ПЕРЕЧИСЛЕНИЯ - красивые маркеры
    t = re.sub(r'([:;.,)\]])\s*(?=[\-–—*•]\s+)', r'\1\n\n• ', t)
    t = re.sub(r'([:;.,)\]])\s*(?=\d+[\.\)]\s+)', r'\1\n\n', t)

    # 4) РАЗРЫВ ДЛИННЫХ ПРЕДЛОЖЕНИЙ - лучшая читаемость
    t = re.sub(r'(?<=\.)\s+(?=[A-ZА-ЯЁ])', '\n\n', t)
    t = re.sub(r'(?<=\?)\s+(?=[A-ZА-ЯЁ])', '\n\n', t)
    t = re.sub(r'(?<=!)\s+(?=[A-ZА-ЯЁ])', '\n\n', t)

    # 5) СПЕЦИАЛЬНЫЕ ФРАЗЫ - выделяем важные моменты
    t = re.sub(r'(?<=\S)\s*(?=(?:ВАЖНО|Внимание|Примечание|Обратите внимание)[^а-яё])', '\n\n⚠️ ', t)
    t = re.sub(r'(?<=\S)\s*(?=(?:Например|К примеру|В частности)[^а-яё])', '\n\n💡 ', t)

    # 6) ПРАВОВЫЕ ТЕРМИНЫ - подчеркиваем структуру
    t = re.sub(r'(?<=\S)\s*(?=Статья\s+\d+)', '\n\n📋 ', t)
    t = re.sub(r'(?<=\S)\s*(?=(?:Согласно|В соответствии с|На основании)[^а-яё])', '\n\n⚖️ ', t)

    # 7) СПИСКИ С ДВОЕТОЧИЕМ - структурируем
    t = re.sub(r'(?<=:)\s*(?=[а-яё•\-])', '\n\n• ', t)
    t = re.sub(r'(?<=;)\s*(?=[а-яё•\-])', '\n• ', t)

    # 8) РАЗБИВАЕМ СКЛЕЕННЫЕ СЛОВА
    t = re.sub(r'(?<=[а-яё])(?=[А-ЯЁ][а-яё])', ' ', t)
    t = re.sub(r'(?<=[a-z])(?=[A-Z][a-z])', ' ', t)

    # 9) УБИРАЕМ ЛИШНИЕ ПЕРЕНОСЫ
    t = re.sub(r'\n{3,}', '\n\n', t)
    # -----------------------------------------
    ...
    for line in lines:
        ...
        if _is_heading(s):
            ...
            # Добавляем эмодзи для разных типов заголовков
            if re.match(r"^\d+\s*[Бб]лок", s2, re.IGNORECASE):
                s2 = f"📚 {s2}"
            elif any(keyword in s2.lower() for keyword in ["краткий ответ", "кратко"]):
                s2 = f"⚡ {s2}"
            elif any(keyword in s2.lower() for keyword in ["подробный", "детально", "разбор"]):
                s2 = f"📋 {s2}"
            elif re.match(r"^[▶️⚖️💡⚠️📋]", s2):
                pass  # уже есть эмодзи
            elif s2.endswith(":"):
                s2 = f"🔸 {s2}"
        ...
        # "Термин: пояснение" -> жирный термин + красивое оформление
        m_term = re.match(r"^([^:\n]{3,80}:\s*)(.*)$", e)
        if m_term:
            term = m_term.group(1).strip()
            explanation = m_term.group(2).strip()

            # Добавляем эмодзи для разных типов терминов
            if any(keyword in term.lower() for keyword in ["статья", "пункт", "часть"]):
                e = f"📋 <b>{term}</b> {explanation}"
            elif any(keyword in term.lower() for keyword in ["как оформить", "порядок", "процедура"]):
                e = f"📝 <b>{term}</b> {explanation}"
            elif any(keyword in term.lower() for keyword in ["преимущества", "плюсы", "достоинства"]):
                e = f"✅ <b>{term}</b> {explanation}"
            elif any(keyword in term.lower() for keyword in ["недостатки", "минусы", "риски"]):
                e = f"⚠️ <b>{term}</b> {explanation}"
            elif any(keyword in term.lower() for keyword in ["нормативная база", "законодательство"]):
                e = f"⚖️ <b>{term}</b> {explanation}"
            else:
                e = f"🔹 <b>{term}</b> {explanation}"

...
ALLOWED_TAGS = ("b", "i", "u", "a", "code", "pre", "blockquote", "span")  # spoiler поддерживается span class="tg-spoiler"
...
def _build_nav_keyboard(sections: List[Section], req_id: str) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(text=f"• {s.title}", callback_data=f"sec:{req_id}:{s.id}")] for s in sections]
...
HEAD_RE = re.compile(r"^\s*(?:\d+\)\s+|[•\-]\s+|—\s+)?(?P<title>(tl;dr|тл;др|итоги|что сделать|шаги|куда обращаться|шаблон|правовые основания).{0,40})$", re.I)
...
class RichReply:
    ...
    async def send_article(self, message: Message, article: Article):
        ...
        await self.session_store.set(f"article:{article.request_id}", {
            "sections": [{"id": s.id, "title": s.title, "html": s.html} for s in article.sections]
        })
        kb = _build_nav_keyboard(article.sections, article.request_id)
        await message.answer(article.tldr_html, parse_mode=ParseMode.HTML, reply_markup=kb, disable_web_page_preview=True)
    ...
    async def handle_callback(self, call: CallbackQuery):
        ...
        try:
            ...
            await call.answer("Неизвестная секция", show_alert=False); return

        data: Optional[Dict] = await self.session_store.get(f"article:{req_id}")
        if not data:
        ...
        if not sec:
            await call.answer("Секция не найдена", show_alert=False); return

...
from .exceptions import ErrorHandler, ErrorContext, ErrorType
...
class BackgroundTask(ABC):
        ...
        while retry_count <= self.max_retries:
            try:
                ...
                if retry_count > self.max_retries:
                    logger.error(f"Background task '{self.name}' failed after {self.max_retries} retries: {e}")
                    return TaskResult(
                    ...
                    logger.warning(f"Background task '{self.name}' failed (retry {retry_count}/{self.max_retries}): {e}")
    ...
    async def _run_loop(self) -> None:
        ...
        logger.info(f"Starting background task '{self.name}' with interval {self.interval_seconds}s")
...
class HealthCheckTask(BackgroundTask):
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        for name, component in self.components.items():
            try:
                ...
                # Пытаемся получить статистику/здоровье компонента
                    ...
                    stats_method = getattr(component, 'get_stats')
                    ...
                    ping_method = getattr(component, 'ping')
        ...
        logger.info(
            f"Health check: {results['healthy_components']}/{total} components healthy"
        )

...
class InputValidator:
    ...
    @classmethod
    def validate_question(cls, text: Optional[str], user_id: Optional[int] = None) -> ValidationResult:
        """
        ...
        if not text:
            return ValidationResult(
                is_valid=False,
                errors=["Текст вопроса не может быть пустым"]
            )
    ...
    def validate_user_id(cls, user_id: Any) -> ValidationResult:
        ...
        if not user_id:
            return ValidationResult(
                is_valid=False,
                errors=["User ID не может быть пустым"]
            )
        ...
        try:
            ...
            if uid <= 0:
                return ValidationResult(
                    is_valid=False,
                    errors=["User ID должен быть положительным числом"]
                )
            ...
            if uid > 2147483647:  # Max 32-bit signed int
                return ValidationResult(
                    is_valid=False,
                    errors=["User ID превышает максимальное значение"]
                )
            
            return ValidationResult(is_valid=True, cleaned_data=uid)
            
        except (ValueError, TypeError):
            return ValidationResult(
                is_valid=False,
                errors=["User ID должен быть числом"]
            )
    ...
    def validate_payment_amount(cls, amount: Any, currency: str = "RUB") -> ValidationResult:
        ...
        try:
            ...
            if amt <= 0:
                return ValidationResult(
                    is_valid=False,
                    errors=["Сумма платежа должна быть больше нуля"]
                )
            ...
            return ValidationResult(is_valid=True, cleaned_data=amt)
        ...
        try:
            if expected_type == bool:
                ...
                if cleaned_value < 0:
                    return ValidationResult(
                        is_valid=False,
                        errors=[f"Значение {key} должно быть неотрицательным"]
                    )
                ...
                if not cleaned_value:
                    return ValidationResult(
                        is_valid=False,
                        errors=[f"Значение {key} не может быть пустым"]
                    )
            else:
                cleaned_value = expected_type(value)
            
            return ValidationResult(is_valid=True, cleaned_data=cleaned_value)
            
        except (ValueError, TypeError) as e:
            return ValidationResult(
                is_valid=False,
                errors=[f"Неверный тип значения для {key}: {str(e)}"]
            )
        ...
        async def wrapper(*args, **kwargs):
            ...
            if args:
                ...
                if not validation_result.is_valid:
                    raise ValidationError(f"Ошибка валидации: {', '.join(validation_result.errors)}")

...
class PMFMetrics:
    ...
    async def get_nps(self, days: int = 30) -> NPSResult:
        ...
        async with self.db.pool.acquire() as conn:
            ...
            if prev_rows:
                ...
                previous_nps = (
                    (prev_promoters / prev_total) - (prev_detractors / prev_total)
                ) * 100
    ...
    async def get_feature_pmf(self, feature_name: str, days: int = 30) -> FeaturePMF:
        ...
        async with self.db.pool.acquire() as conn:
            ...
            pmf_score = (
                satisfaction_norm * 0.4 + frequency_norm * 0.3 + retention_norm * 0.3
            ) * 100
    ...
    async def record_nps_response(
        ...
        user_id: int,
        score: int,
        disappointment_level: str | None = None,
...
_PMF_METRIC_METHODS = (PMFMetrics.record_nps_response,)

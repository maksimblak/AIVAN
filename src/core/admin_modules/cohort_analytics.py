...
class CohortAnalytics:
    ...
    async def _get_cohort_feature_adoption(
        self, conn, cohort_month: str
    ...
    def _generate_insights(
        self, cohorts: list[CohortMetrics], best: CohortMetrics, worst: CohortMetrics
    ) -> list[str]:
        ...
        if latest_conversion > avg_conversion * 1.1:
            insights.append(
                f"✅ Последняя когорта конвертится лучше среднего ({latest_conversion:.1f}% vs {avg_conversion:.1f}%)"
            )
        elif latest_conversion < avg_conversion * 0.9:
            insights.append(
                f"⚠️ Последняя когорта конвертится хуже среднего ({latest_conversion:.1f}% vs {avg_conversion:.1f}%)"
            )
    ...
    async def get_feature_adoption_by_cohort(
        self, feature_name: str, months_back: int = 6
    ) -> FeatureAdoptionCohort:
        """

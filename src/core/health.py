    ...
    async def check(self) -> HealthCheckResult:
        try:
            ...
            return HealthCheckResult(
                status=status,
                message=message,
                details={**stats, "cache_status": cache_status}
            )
            ...
            while self._running:
                try:
                    ...
                    for name, result in results.items():
                        if result.status != HealthStatus.HEALTHY:
                            level = logging.WARNING if result.status == HealthStatus.DEGRADED else logging.ERROR
                            logger.log(level, f"Health check '{name}': {result.status.value} - {result.message}")

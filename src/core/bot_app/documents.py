...
def _with_back_to_operations(markup: InlineKeyboardMarkup | None = None) -> InlineKeyboardMarkup:
    """Append a back-to-operations button to existing markup (or create new one)."""
    back_button = InlineKeyboardButton(text="⬅️ Назад", callback_data="document_processing")
...
_STAGE_LABEL_OVERRIDES: dict[str, dict[str, tuple[str, str]]] = {
    "summarize": {
        ...
        "summary_generation": ("Собираем ключевые тезисы", "✨"),
    ...
    "analyze_risks": {
        ...
        "ai_analysis": ("Проверяем дополнительные сценарии", "🧠"),
    ...
    "anonymize": {
        ...
        "text_extracted": ("Извлекаем текст", "📄"),
        "ai_prepare": ("Готовим расширенный анализ", "🧭"),
        "ai_chunk": ("Обрабатываем фрагменты", "🧠"),
...
async def _finalize_draft(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        footer_parts = [
            f"{Emoji.MAGIC} Документ успешно создан",
            "📎 Формат: DOCX",
    ...
    try:
        ...
        detailed_descriptions = {
            ...
            "lawsuit_analysis": (
                ...
                "━━━━━━━━━━━━\n\n"
                "🧠 <b>Что получите:</b>\n\n"
                "📌 <b>Оценка аргументов</b>\n"
                "   └ Анализ сильных сторон вашей позиции\n"
                "   └ Выявление слабых мест и рисков\n\n"
                "📚 <b>Судебная практика</b>\n"
                "   └ Подборка аналогичных дел\n"
                "   └ Анализ исходов по похожим спорам\n\n"
                "🛠️ <b>Практические советы</b>\n"
                "   └ Как усилить позицию\n"
                    ...
                    for export in exports:
                        ...
                        try:
                            ...
                            if not back_button_sent:
                                export_markup = _with_back_to_operations()
                                back_button_sent = True
                            await message.answer_document(

...
class DatabaseException(BaseCustomException):
    """Ошибки базы данных"""
    def __init__(self, message: str, context: Optional[ErrorContext] = None, is_connection_error: bool = False):
...
class OpenAIException(BaseCustomException):
    """Ошибки OpenAI API"""
    def __init__(self, message: str, context: Optional[ErrorContext] = None, is_quota_error: bool = False):
        severity = ErrorSeverity.HIGH if is_quota_error else ErrorSeverity.MEDIUM
        user_msg = "Превышен лимит запросов к ИИ. Попробуйте позже." if is_quota_error else "Проблемы с ИИ-сервисом. Попробуйте позже."
        super().__init__(
            message, 
...
class PaymentException(BaseCustomException):
    """Ошибки платежей"""
    def __init__(self, message: str, context: Optional[ErrorContext] = None, is_user_error: bool = False):
...
class ErrorHandler:
    ...
    async def handle_exception(
        ...
        self.error_counts[custom_exc.error_type] = self.error_counts.get(custom_exc.error_type, 0) + 1
        ...
        if custom_exc.recoverable and custom_exc.error_type in self.recovery_handlers:
            try:
                ...
                self.logger.error(f"Failed to recover from {custom_exc.error_type}: {recovery_error}")
    ...
    def _classify_exception(
        ...
        if isinstance(exception, (ConnectionError, TimeoutError)):
            ...
            is_conn_error = "database is locked" in exc_message.lower() or "connection" in exc_message.lower()
...
def handle_exceptions(error_handler: ErrorHandler, context_func: Optional[Callable] = None):
